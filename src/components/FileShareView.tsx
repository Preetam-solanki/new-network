...
import { SharedFile, Network } from '../types';
...
export const FileShareView: React.FC<FileShareViewProps> = ({
  ...
  const smbPath = `\\\\${activeNetwork?.assignedVirtualIp || '10.147.19.1'}\\SharedLAN`;
  const nfsPath = `smb://${activeNetwork?.assignedVirtualIp || '10.147.19.1'}/SharedLAN`;
      ...
      setDownloadProgress((prev) => {
        if (prev >= 100) {
          ...
          setTimeout(() => {
            ...
            const dummyBlob = new Blob([`V-WAN Virtual LAN P2P transfer payload for: ${file.name}\nSHA-256: ${file.sha256}`], { type: 'text/plain' });
  ...
  const filteredFiles = files.filter(
    (f) =>
      (!selectedNetworkId || f.networkId === selectedNetworkId) &&
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <div className="flex items-center gap-2">
              <h2 className="text-base font-extrabold text-white flex items-center gap-2">
                ...
                <span>P2P Virtual File Sharing & SMB Gateway</span>
              </h2>
            ...
            <p className="text-xs text-slate-400 mt-1 max-w-2xl">
              High-throughput encrypted tunnel transfer. Share game mods, world saves, ROMs, and project files directly with virtual peers without third-party cloud upload limits.
            </p>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              <span>Share File to Network</span>
            </button>
          </div>
        </div>
        ...
        <div className="mt-3.5 pt-3 border-t border-slate-800 grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
          <div className="bg-slate-850 p-2.5 rounded-lg border border-slate-800 flex items-center justify-between">
            <div className="space-y-0.5">
              <span className="text-slate-400 text-[11px] block font-medium">🪟 Windows Explorer SMB Share:</span>
          ...
          <table className="w-full text-left text-xs">
            <thead className="bg-slate-950/70 border-b border-slate-800 text-slate-400 uppercase text-[10px] font-semibold">
              <tr>
                ...
                <th className="px-4 py-3">Shared By (VIP)</th>
              ...
              {filteredFiles.length === 0 ? (
                <tr>
                  <td colSpan={6} className="px-4 py-8 text-center text-slate-500 italic">
                    No files shared in this network yet. Click "Share File to Network" above.
                  </td>
                </tr>
              ) : (
                filteredFiles.map((file) => {
                  ...
                  return (
                    <tr key={file.id} className="hover:bg-slate-850/60 transition-colors">
                      ...
                      <td className="px-4 py-3.5">
                        <div>
                          <span className="block font-medium text-slate-200">{file.uploaderName}</span>
                      ...
                      <td className="px-4 py-3.5 text-right">
                        {isDownloading ? (
                          ...
                          <button
                            ...
                            className="px-3 py-1.5 rounded-lg bg-emerald-600/20 hover:bg-emerald-600/30 text-emerald-300 border border-emerald-500/40 text-xs font-semibold flex items-center gap-1.5 ml-auto transition-colors cursor-pointer"
                          >
